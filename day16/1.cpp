...
int version_sum(const packet_t& structure) {
  int sum = structure.version;
  for (const auto& item : structure.subpackets) {
    sum += version_sum(item);
  }
  return sum;
}
...
int main() {
  ...
  std::cout << version_sum(structure) << std::endl;
