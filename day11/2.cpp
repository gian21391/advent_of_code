...
void execute_step(std::vector<std::vector<int>>& map) {
  for (auto& items : map) {
    for (auto& item : items) {
      ++item;
    }
  ...
  if (to_be_increased.empty()) return;
...
int main() {
  ...
  int i = 0;
  while (!std::ranges::all_of(map, [](const auto& item){ return std::ranges::all_of(item, [](const auto& i){ return i == 0; }); })) {
    i++;
    execute_step(map);
  }
